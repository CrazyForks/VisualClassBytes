...
import com.liubs.visual.classbytes.ui.common.TextIcon;
...
public class MyLineGutterRenderer extends GutterIconRenderer {
    ...
    public static List<MyLineNumber> getAllMarkedLines(Editor editor) {
        List<MyLineNumber> markedLines = new ArrayList<>();
        MarkupModel markupModel = editor.getMarkupModel();

        for (RangeHighlighter highlighter : markupModel.getAllHighlighters()) {
            if (Boolean.TRUE.equals(highlighter.getUserData(ASM_LABEL_LINE))) {
                GutterIconRenderer renderer = highlighter.getGutterIconRenderer();
                if (renderer instanceof MyLineGutterRenderer) {
                    MyLineGutterRenderer myRenderer = (MyLineGutterRenderer) renderer;
                    MyLineNumber myLineNumber = new MyLineNumber();
                    myLineNumber.setLabelIndex(myRenderer.labelIndex);
                    myLineNumber.setLineEditor(myRenderer.editorLine);
                    myLineNumber.setLineSource(myRenderer.sourceCodeLine);
                    markedLines.add(myLineNumber);
                }
            }
        }
        return markedLines;
